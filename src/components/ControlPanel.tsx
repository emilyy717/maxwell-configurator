      ...
      <div className="flex-grow space-y-4">
        ...
        {selectedCamera ? (
          <div className="space-y-4 bg-gray-900/50 p-4 rounded-lg">
            ...
            <SliderControl 
              label="Position X"
              value={selectedCamera.x / pixelsPerMm}
              onChange={(val) => onUpdateCamera(selectedCamera.id, { x: val * pixelsPerMm })}
              min={-40}
              max={40}
              step={0.5}
              unit="mm"
            />
             <SliderControl 
              label="Position Y"
              value={selectedCamera.y / pixelsPerMm}
              onChange={(val) => onUpdateCamera(selectedCamera.id, { y: val * pixelsPerMm })}
              min={-30}
              max={30}
              step={0.5}
              unit="mm"
